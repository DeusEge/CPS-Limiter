...
Window::Window() {
    for (int i = 0; i < devices.size(); i++) {
        event_actions.push_back(new QAction((devices[i].eventPath + "    " + devices[i].name).c_str(), this));
        event_actions[i]->setCheckable(true);
        if (devices[i].selected) event_actions[i]->setChecked(true);
        connect(event_actions[i], &QAction::triggered, this, [this, i]() {
            device = devices[i].eventPath;
            for (int i = 0; i < this->event_actions.size(); i++) this->event_actions[i]->setChecked(false);
            this->event_actions[i]->setChecked(true);
        });
    }
...
void Window::create_widgets() {
    ...
    // menu ---
    {
        ...
        event_menu = new QMenu("Device", this);
    ...
    // mouse pixmap ---
    {
        ...
        mouse_left_click->move(5 * window_multiply, 10 * window_multiply);
        ...
        mouse_right_click->move(5 * window_multiply, 10 * window_multiply);
...
void Window::run_button_clicked() {
    ...
    for (int i = 0; i < event_actions.size(); i++) event_actions[i]->setEnabled(false);
...
void Window::stop_button_clicked() {
    ...
    stop_button->setEnabled(false);
    for (int i = 0; i < event_actions.size(); i++) event_actions[i]->setEnabled(true);
