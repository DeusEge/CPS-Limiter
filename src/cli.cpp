...
int main(int argc, char* argv[]) {
    ...
    if (argc == 1) {
        std::cout << "usage: sudo ./cps-limiter-cli </dev/input/eventX> <left-limit> <left-cooldown> <right-limit> <right-cooldown>" << std::endl;
        std::cout << "install and use evtest to learn event index" << std::endl;
        std::cout << "running with default settings" << std::endl;
    }
    else if (argc == 6) {
        event = argv[1];
        left::cps_limit = std::stoi(argv[2]);
        left::cooldown = std::stoi(argv[3]);
        right::cps_limit = std::stoi(argv[4]);
        right::cooldown = std::stoi(argv[5]);
    }
    else {
        std::cout << "usage: sudo ./cps-limiter-cli </dev/input/eventX> <left-limit> <left-cooldown> <right-limit> <right-cooldown>" << std::endl;
        std::cout << "install evtest and run sudo evtest to learn event index" << std::endl;
        return -1;
    }
